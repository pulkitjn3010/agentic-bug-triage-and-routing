...
async def get_metrics(user_id: str | None = None) -> dict:
    ...
    avg_confidence = (
        round(total_confidence / confidence_count, 2) if confidence_count else 0
    )
    ...
    triaged_today = sum(
        1 for e in recent if e.created_at and e.created_at >= today_start
    )
        ...
        "recent_activity": [
            {
                ...
                "severity": (
                    ...
                    or (e.summary or {}).get("severity", "Unknown")
                ),
