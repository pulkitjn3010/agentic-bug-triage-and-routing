...
async def get_next_group_id(db: AsyncSession) -> str:
    result = await db.execute(select(func.count(SystemGroupRegistry.group_id)))
...
async def get_group_for_ticket(
    db: AsyncSession, raw_ticket_id: str, source_id: str
) -> str | None:
...
async def get_group_for_any_ticket(db: AsyncSession, tickets: list[dict]) -> str | None:
    for t in tickets:
        gid = await get_group_for_ticket(
            db, t.get("ticket_id", ""), t.get("source_id", "")
        )
        if gid:
            return gid
    return None
...
async def add_tickets_to_group(
    db: AsyncSession, group_id: str, tickets: list[dict]
) -> None:
    for t in tickets:
...
async def get_tickets_in_group(
    db: AsyncSession, group_id: str
) -> list[BugGroupMapping]:
    result = await db.execute(
        select(BugGroupMapping).where(BugGroupMapping.group_id == group_id)
    )
...
def _ticket_ref(ticket: dict, role: str) -> dict:
    return {
        "ticket_id": (
            ticket.get("ticket_id") or ticket.get("id") or ticket.get("key") or ""
        ),
        "source_id": (
        ...
        "system_type": (ticket.get("system_type") or ticket.get("source") or ""),
        "role": role,
...
async def persist_related_issue_group(
    ...
    child_refs = [
        ...
        if (ticket.get("similarity_score") or ticket.get("relevance_score") or 0)
        >= 0.50
    ]
