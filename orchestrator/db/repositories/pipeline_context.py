...
async def create_pipeline_context(
    db: AsyncSession, case_id: str, context_json: dict | None = None
) -> PipelineContext:
    ctx = PipelineContext(
        case_id=case_id, current_step="start", context_json=context_json or {}
    )
    db.add(ctx)
    await db.commit()
...
async def get_pipeline_context(
    ...
    result = await db.execute(
        select(PipelineContext).where(PipelineContext.case_id == case_id)
    )
    return result.scalar_one_or_none()


async def update_pipeline_step(
    db: AsyncSession, case_id: str, step: str, context_json: dict | None = None
) -> None:
    ...
    await db.execute(
        ...
        .where(PipelineContext.case_id == case_id)
        .values(**values)
    )
    await db.commit()
