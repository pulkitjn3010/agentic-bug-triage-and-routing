...
class TaskOrchestrator:
    ...
    async def _run_pipeline(
        ...
        log.info(
            "Orchestrator context initialized",
            case_id=case_id,
            ticket_id=bug_id,
            source_id=source_id,
        )
        ...
        if "cross_system_fetch" in steps_to_run or "enrichment" in steps_to_run:
            ...
            try:
                ...
                await _r.setex(
                    f"enrichment:{case_id}",
                    120,
                    _json.dumps(context.get("enrichment_sources") or []),
                )
                await _r.setex(
                    f"kb:{case_id}", 120, _json.dumps(context.get("kb_articles") or [])
                )
            except Exception:
                pass
        ...
        async with AsyncSessionLocal() as db:
            db_entry = await insert_audit_entry(
                db,
                {
                    "case_id": case_id,
                    "bug_id": bug_id,
                    ...
                    "summary": {
                        ...
                        "status": (context.get("primary_ticket") or {}).get(
                            "status", ""
                        ),
                        "updated_at": (context.get("primary_ticket") or {}).get(
                            "updated_at", ""
                        ),
                    },
    ...
    async def _handle_infrastructure_failure(
        ...
        try:
            ...
            log.warning(
                "Infra failure: context delete failed", case_id=case_id, error=str(e)
            )
        ...
        try:
            ...
            log.warning(
                "Infra failure: error panel publish failed",
                case_id=case_id,
                error=str(e),
            )
        ...
        try:
            ...
            log.warning(
                "Infra failure: audit write failed", case_id=case_id, error=str(e)
            )
    ...
    async def _sweep_stale_contexts(self) -> None:
        try:
            ...
            if result.rowcount:
                log.info(
                    "Swept stale pipeline_context rows",
    ...
    async def _publish_panel(
        ...
        try:
            ...
            log.warning("Panel publish failed", panel=panel_name, error=str(e))

    async def _publish_complete(
        self, case_id: str, synthesis: dict, duration_ms: int
    ) -> None:
        try:
            await publish_pipeline_done(
                case_id=case_id,
    ...
    def _missing_ai_requirements(self, context: dict) -> list[str]:
        ...
        if "related_tickets" not in context and "related_issues" not in context:
            missing.append("related_tickets")
        if "kb_articles" not in context and "knowledge_base" not in context:
    ...
    async def _persist_related_group(self, context: dict) -> str | None:
        ...
        try:
            ...
            log.warning(
                "Related group persistence failed",
                case_id=context.get("case_id"),
                error=str(e),
            )
            return None
    ...
    def _audit_related_tickets(self, related: list[dict]) -> list[dict]:
        ...
        for item in related[:20]:
            output.append(
                {
                    "ticket_id": (
                        item.get("ticket_id") or item.get("id") or item.get("key") or ""
                    ),
                    "source": (
                        item.get("source")
                    ...
                    "similarity_score": (
                        item.get("similarity_score") or item.get("relevance_score")
                    ),
                    "similarity_label": item.get("similarity_label", ""),
                    "similarity_reason": (item.get("similarity_reason", "")[:300]),
                }
            )
        return output
