...
class ContextFetchAgent(BaseAgent):
    ...
    async def run(self, context: dict) -> dict:
        ...
        log.info("ContextFetch start", bug_id=bug_id, source_id=source_id)

        connector = await self._resolve_connector(source_id, bug_id)

        if connector is None:
            log.error(
                "ContextFetch: no connector found", bug_id=bug_id, source_id=source_id
            )
            self._add_error(context, f"No connector resolved for bug_id={bug_id}")
        ...
        log.info(
            "ContextFetch: connector resolved",
        ...
        ticket = None
        try:
            ticket = await asyncio.wait_for(connector.get_ticket(bug_id), timeout=15.0)
        except asyncio.TimeoutError:
            log.error("ContextFetch: GET timed out", bug_id=bug_id)
            self._add_error(context, f"Timeout fetching {bug_id}")
        except Exception as e:
            log.error("ContextFetch: GET error", bug_id=bug_id, err=str(e))
            self._add_error(context, str(e))

        if ticket is None:
            log.error(
                "ContextFetch: ticket is None",
                bug_id=bug_id,
                connector=connector.source_id,
            )
            context["primary_ticket"] = None
        ...
        if co_refs:
            log.info(
                "ContextFetch: co-refs found", count=len(co_refs), refs=co_refs[:3]
            )
        ...
        try:
            portals = await ConnectorRegistry.get_all_by_type("customer_portal")
            if portals:
                q = (
                    f"{ticket.title} " f"{ticket.component or ''} " f"{err or ''}"
                ).strip()
                ...
                gathered = await asyncio.gather(
                    *[
                        asyncio.wait_for(
                            portal.search(
                                q, max_results=3, primary_ticket=primary_signal_query
                            ),
                            timeout=5.0,
                        )
                ...
                for portal, item in zip(portals, gathered):
                    ...
                    results.extend(item)
                customer_cases = [self._normalize_customer_signal(t) for t in results]
                log.info(
                    "ContextFetch: customer cases",
    ...
    def _normalize_ticket(
        ...
        ticket_dict["linked_items"] = self._normalize_linked_items(
            connector=connector,
            linked_items=linked_items or (ticket_dict.get("linked_items") or []),
        )
        ticket_dict["source_id"] = connector.source_id
        ticket_dict["source"] = ticket_dict.get("system_type") or connector.system_type
    ...
    def _build_bug_context(
        self, ticket: dict, customer_cases: list, source_references: list, errors: dict
    ) -> dict:
        return {
    ...
    def _empty_bug_context(
        self, bug_id: str, source_id: str = "", error: str = ""
    ) -> dict:
        return {
            "ticket_id": bug_id or "",
    ...
    def _normalize_linked_items(self, connector, linked_items: list) -> list:
        ...
        for item in linked_items or []:
            if not isinstance(item, dict):
                continue
            copy = dict(item)
            copy["url"] = copy.get("url") or self._linked_item_url(connector, copy)
    ...
    def _build_source_references(
        self, ticket, linked_items: list, co_refs: list
    ) -> list:
        references = []
        for item in linked_items or []:
            references.append(
        ...
        for word in words:
            ...
            if "-" in w:
                ...
                if (
                    ...
                    refs.append({"raw_id": w, "source": "JIRA", "type": "co_reference"})
            # GitHub PR/issue
            if w.upper().startswith(("PR-", "GH-")):
                parts = w.split("-")
                if len(parts) == 2 and parts[1].isdigit():
                    refs.append(
                        {"raw_id": parts[1], "source": "GitHub", "type": "co_reference"}
                    )
        # Remove duplicates
        seen = set()
        unique = []
