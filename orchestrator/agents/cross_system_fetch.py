...
class CrossSystemFetchAgent(BaseAgent):
    ...
    async def run(self, context: dict) -> dict:
        ...
        keywords = list(
            dict.fromkeys(
                ...
                if len(w.strip(".,()[]")) > 3 and w.lower().strip(".,()[]") not in _stop
            )
        )[:8]
        ...
        github_base = next(
            (c.base_url for c in all_connectors if c.system_type == "github"),
            "https://api.github.com",
        )
        github_repo = next(
            (c.project_key for c in all_connectors if c.system_type == "github"), ""
        )
        bugzilla_base = next(
            (c.base_url for c in all_connectors if c.system_type == "bugzilla"), ""
        )
        ...
        for gh_id in backlinks["github"]:
            url = (
                f"https://github.com/{github_repo}/issues/{gh_id}"
                if github_repo
                else ""
            )
            candidates.append(
        ...
        for bz_id in backlinks["bugzilla"]:
            url = f"{bugzilla_base}/show_bug.cgi?id={bz_id}" if bugzilla_base else ""
            candidates.append(
                {
                    "id": f"BZ-{bz_id}",
                    "source": "bugzilla",
        ...
        if cache_candidates:
            ...
            scored = await self._batch_score_with_groq(source_ticket, cache_candidates)
        ...
        live_results, sources_queried = await self._live_api_search(
            primary, primary_source, context, all_connectors=all_connectors
        )
        ...
        normalized.sort(key=lambda x: x.get("similarity_score", 0.0), reverse=True)
    ...
    def _normalize_candidate(self, raw: dict) -> dict:
        ...
        """
        id_ = raw.get("id") or raw.get("ticket_id") or raw.get("key") or ""
        source = (
            raw.get("system_type")
            or raw.get("source")
        ...
        if "github" in (source or "").lower() and id_ and str(id_).isdigit():
            id_ = f"#{id_}"

        url = raw.get("url") or raw.get("html_url") or raw.get("link") or ""
        ...
        score = round(
            max(0.0, min(1.0, raw_score / 10.0 if raw_score > 1.0 else raw_score)), 2
        )
        ...
        return {
            ...
            "title": (raw.get("title") or raw.get("summary") or raw.get("name") or ""),
            "url": url,
            "status": status,
            "source": source,
            "source_id": source_id,
            "system_type": source,  # backward-compat alias
            "description": (raw.get("description") or raw.get("body") or "")[:300],
            "relevance_score": score,
            "similarity_score": score,  # alias
            "similarity_label": (raw.get("similarity_label") or ""),
            "similarity_reason": (
                raw.get("similarity_reason") or raw.get("reason") or ""
            ),
            "relationship_type": (
    ...
    async def _scan_redis_cache(self, keywords: list[str]) -> list[dict]:
        # Strip stop-words before scanning — prevents noise matches on
        # generic terms like "error", "fix", "update".
        meaningful_keywords = [
            k for k in keywords if k.lower() not in STOP_WORDS and len(k) > 2
        ]
        if not meaningful_keywords:
            log.warning(
                "CrossSystem: all keywords were stop-words, " "skipping redis scan"
            )
            return []
            ...
            for key in keys:
                try:
                    ...
                    for bug in bug_list:
                        ...
                        overlap = sum(1 for kw in meaningful_keywords if kw in text)
                        if overlap >= 1:
                            bug_id = bug.get("ticket_id") or bug.get("id") or ""
                            system_type = (
                                bug.get("system_type")
                                or bug.get("source_id")
                                or "unknown"
                            )
                            ...
                            hits.append(
                                {
                                    ...
                                    "description": (bug.get("description") or "")[:300],
                                    "source": system_type,
                                    "system_type": system_type,
                                    "url": clean_url,
                    ...
                    continue
            hits.sort(key=lambda x: x["overlap_score"], reverse=True)
    ...
    async def _batch_score_with_groq(
        self, source_ticket: dict, candidates: list[dict]
    ) -> list[dict]:
        ...
        if True:
            ...
            if not result:
                log.info(
                    "All Level 2 candidates scored below "
        ...
        try:
            ...
            for c in candidates:
                sid = str(c["id"])
                if sid in score_map:
                    c["relevance_score"] = _normalize(score_map[sid]["relevance_score"])
            ...
            return [_alias_fields(c) for c in scored_candidates[:10]]

        except Exception as e:
            log.warning("CrossSystem Groq cache scoring failed", error=str(e))
            result = []
            for c in candidates:
    ...
    async def _live_api_search(
        ...
        log.info("CrossSystem queries", queries=query_map, source=primary_source)
        ...
        desc_query = self._extract_description_keywords(
            primary.get("description", ""), primary.get("title", "")
        )
        ...
        for source_id, tickets in gathered:
            ...
            log.info(
                "CrossSystem external source results",
                source=source_id,
                count=len(tickets),
            )

        log.info(
            "CrossSystem candidates",
        ...
        if not live_candidates and not direct_hits:
            ...
            fallback_map = self._deterministic_fallback(
                primary_title, primary.get("component") or ""
            )
            fallback_term = fallback_map.get("jira_query", "")

            if fallback_term and fallback_term != query_map.get("jira_query", ""):
                log.info("CrossSystem Tier2 fallback", term=fallback_term)
                ...
                fb = await asyncio.gather(
                    *[search_fallback(c) for c in targets], return_exceptions=True
                )
                for result in fb:
        ...
        log.info("CrossSystem candidates after fallback", total=len(live_candidates))
        ...
        for d in scored:
            ...
            if d.get("from_co_reference"):
                ...
                d["similarity_score"] = round(min(1.0, raw_score + 0.1), 2)
                if d.get("similarity_reason"):
                    d["similarity_reason"] += " (explicitly referenced in bug text)"
...
{{
  ...
  "component_error_query": "component + error/symptom 2-3 words"
        ...
        try:
            ...
            result = {
                "jira_query": str(parsed.get("jira_query", "")).strip()[:80],
                "github_query": str(parsed.get("github_query", "")).strip()[:80],
                "bugzilla_query": str(parsed.get("bugzilla_query", "")).strip()[:80],
                "broad_query": str(parsed.get("broad_query", "")).strip()[:80],
                "component_error_query": str(
                    parsed.get("component_error_query", "")
                ).strip()[:80],
            }

            for key in ["jira_query", "github_query", "bugzilla_query"]:
                first = result[key].split()[0].lower() if result[key] else ""
                if not result[key] or first in generic:
                    result[key] = fallback.get(key, "")

            if not result["broad_query"]:
                result["broad_query"] = component or fallback["jira_query"]
            ...
            fb = {**fallback, "broad_query": component or fallback["jira_query"]}
            if component:
    ...
    def _deterministic_fallback(self, title: str, component: str) -> dict:
        ...
        file_patterns = re.findall(r"\.[\w]+(?:rc|config|yml|yaml|json|js|ts)", title)
        if file_patterns:
            # Strip leading dot
        ...
        if technical_words:
            ...
            fallback_words = [
                w for w in words if len(w) > 4 and w.lower() not in STOP_WORDS
            ]
    ...
    def _select_targets(self, all_connectors: list, primary_source_id: str) -> list:
        ...
        candidates = [
            c
            for c in all_connectors
            if c.source_id != primary_source_id and getattr(c, "is_bug_source", False)
        ]
        if not candidates:
            return []
    ...
    def _extract_backlinks(self, description: str, comments: list[str]) -> dict:
        import re

        combined = " ".join([description] + list(comments))

        jira_ids = set(re.findall(r"\b([A-Z][A-Z0-9]+-\d+)\b", combined))
        github_refs = set(re.findall(r"#(\d+)", combined))
        bugzilla_ids = set(re.findall(r"\bBZ-(\d+)\b", combined))
    ...
    async def _fetch_co_references(
        self, context: dict, all_connectors: list, primary_source: str
    ) -> list:
        ...
        for ref in co_refs[:5]:
            for c in all_connectors:
                if c.source_id == primary_source or not getattr(
                    c, "is_bug_source", False
                ):
                    continue
                try:
                    t = await asyncio.wait_for(c.get_ticket(ref["raw_id"]), timeout=8.0)
                    if t:
                        ...
                        td["similarity_reason"] = "Cross-reference found in bug text"
                        td["similarity_matching_fields"] = ["direct_reference"]
                        hits.append(td)
                        log.info(
                            "CrossSystem co-ref found",
                            ref=ref["raw_id"],
                            source=c.source_id,
    ...
    async def _batch_score(
        self, primary: dict, candidates: list, api_key: str, model: str
    ) -> list:
        if not candidates:
            return []
        ...
        result = [c for c in candidates[:12] if c.get("similarity_score", 0) >= 0.55]
        result.sort(key=lambda x: x.get("similarity_score", 0), reverse=True)
        return result

    # ── Description keyword extraction ──────────────────────────
    def _extract_description_keywords(self, description: str, title: str) -> str:
        ...
        exceptions = re.findall(
            r"\b([A-Z][a-z]+(?:[A-Z][a-z]+)*(?:Exception|Error|" r"Fault|Failure))\b",
            text,
        )
        ...
        files = re.findall(
            ...
            r"properties|json|gradle|scala))\b",
            text,
        )
        ...
        keywords = []
        seen = set()
        for pool in [exceptions[:2], camel[:2], files[:1], tech_words[:3]]:
            for kw in pool:
                kw_lower = kw.lower()
        ...
        log.info("CrossSystem description keywords", keywords=result)
        return result
